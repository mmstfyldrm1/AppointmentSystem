...
namespace AppointmentSystem.ViewComponents.Default
{
    public class _AppointmentSystemCompanyListComponentPartial:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var queryObj = new { query = "select top 6 s.Name,s.ShopImg,s.ShopAddress,s.ShopPhone, so.Name as ShopOwnerName  from Dt_Shops s with(nolock) left join Dt_ShopOwners so with(nolock) on so.Id=s.ShopOwnerId" };
